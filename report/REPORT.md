...
|-----------|----------|----------------------|-----------|----------|----------------------------|
| Tôi (Khánh) | `RecursiveChunker(chunk_size=500)` + `category`/`language` filters | 10/10 | Giữ nguyên cấu trúc ngữ nghĩa khoản/điểm luật; filter metadata loại bỏ nhiễu chéo hiệu quả giữa các bộ luật. | Số lượng chunks lớn hơn, tăng chi phí lưu trữ và thời gian index/truy xuất. | **Core System Engineer:** Thiết lập framework cốt lõi của package `src`, triển khai cấu trúc lớp chunker và embedding store, đồng thời tối ưu hóa logic tiền xử lý và lưu trữ. |
